...
UndoItem::~UndoItem() { DELETEP(clone); }
Document::MyPrintout::MyPrintout(Document *d) : doc(d), wxPrintout(L"printout") {}
...
bool Document::MyPrintout::HasPage(int pageNum)
{
    return pageNum == 1;
}
...
Document::Document() : sw(NULL), rootgrid(NULL), pathscalebias(0), filename(L""),
...
void Document::ResetCursor() { if(selected.g) selected.SetCursorEdit(this, selected.TextEdit()); }
...
int Document::TextSize(int depth, int relsize) { return max(g_mintextsize(), g_deftextsize-depth-relsize+pathscalebias); }
...
const char *Document::Export(wxDC &dc, const wxChar *fmt, const wxChar *pat, const wxChar *msg, int k)
{
...
const char *Document::Key(wxDC &dc, wxChar uk, int k, bool alt, bool ctrl, bool shift, bool &unprocessed)
{
    Cell *c = selected.GetCell();
...
char *Document::SearchNext(wxDC &dc)
{
...
void Document::PasteSingleText(Cell *c, const wxString &t)
{
...
int Document::_timesort(const Cell **a, const Cell **b)
{
