...
Image::Image(wxBitmap _bm, int _cs) : bm(_bm), checksum(_cs) {}
void Image::Scale(float sc)
{
    bm = wxBitmap(bm.ConvertToImage().Scale(bm.GetWidth()*sc, bm.GetHeight()*sc, wxIMAGE_QUALITY_HIGH));
}

void System::SaveChecker::Notify() { sys->SaveCheck(); }

System::System() : cfg(L"TreeSheets"), cellclipboard(NULL),
           defaultfont(
            #ifdef WIN32
...
void System::Init(int argc, wxChar **argv)
{
    ev.Init();
...
wxString System::ExtName(const wxString &filename, wxString ext) { wxFileName fn(filename); return fn.GetPathWithSep()+fn.GetName()+ext; }
...
void System::ImageSize(Image *image, int &xs, int &ys)
{
    if(!image) return;
...
void System::ImageDraw(Image *image, wxDC &dc, int x, int y, int xs, int ys)
{
