...
Selection::Selection(Grid *_g, int _x, int _y, int _xs, int _ys) : g(_g), x(_x), y(_y), xs(_xs), ys(_ys), cursor(0), cursorend(0), textedit(false), firstdx(0), firstdy(0) {}
...
bool Selection::EqLoc(const Selection &s) { return g==s.g && x==s.x && y==s.y && xs==s.xs && ys==s.ys; }
bool Selection::operator==(const Selection &s) { return EqLoc(s) && cursor==s.cursor && cursorend==s.cursorend; }

bool Selection::Thin() const { return !(xs*ys); }
...
bool Selection::TextEdit() { return textedit; }
void Selection::EnterEditOnly(Document *doc) { textedit = true; SetCursorEdit(doc, true); }
void Selection::EnterEdit(Document *doc, int c, int ce) { EnterEditOnly(doc); cursor = c; cursorend = ce; }
void Selection::ExitEdit(Document *doc) { textedit = false; cursor = cursorend = 0; SetCursorEdit(doc, false); }
...
char *Selection::Dir(Document *doc, bool ctrl, bool shift, wxDC &dc, int dx, int dy, int &v, int &vs, int &ovs, bool notboundaryperp, bool notboundarypar, bool exitedit)
{
    if(ctrl && !textedit)
    {
        g->cell->AddUndo(doc);
